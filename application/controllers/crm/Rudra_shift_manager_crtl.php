...
{    public  $default_data = array();   
    ...
    public function index()
    {
        ...
        $data['page_template'] = '_rudra_shift_manager';
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'fo_name','sm_name', 'sm_mobile', 'sm_email', 'sm_password', 'status', );
		...
		$totalData = $this->rudram->count_table_data($filter_data,$this->full_table,$leftJoin1);
		...
		$rows_count = $this->rudram->count_table_data($filter_data,$this->full_table,$leftJoin1);
		$totalFiltered = $rows_count;
		if(!empty($rows))
		{
			$res_json = array();
			foreach ($rows as $row)
			{   //$row->sm_name = $row->sm_fname."".$row->sm_lname;
				//$row->fo_name = $row->fo_fname."".$row->fo_lname;
				$actions_base_url = 'shift-manager/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'shift-manager/post_actions';
				$action_url = 'shift-manager/post_actions';
    ...
    public function post_actions($param1)
    {
		...
		$data['fo_data'] = $this->db->get_where('rudra_facility_owner',array('status'=>'Active'))->result(); 
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_shift_manager_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {
				...
				array(
				 ...
				 'sm_fcm_token' => $this->input->post('sm_fcm_token',true),
				 'sm_device_token' => $this->input->post('sm_device_token',true),
