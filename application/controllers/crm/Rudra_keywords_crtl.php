...
class Rudra_keywords_crtl extends CI_Controller
{                   
    public function __construct()
    {
        ...
        $this->load->model('rudra_keywords_rudra_model','rudram');
	...
	//Rudra_keywords_crtl ROUTES
        ...
        $route['rudra_keywords'] = $crud_master . 'index';
        $route['rudra_keywords/index'] = $crud_master . 'index';
        $route['rudra_keywords/list'] = $crud_master . 'list';
        $route['rudra_keywords/post_actions/(:any)'] = $crud_master.'post_actions/$1';
	**************/
    ...
    public function index()
    {
        ...
        $data['page_template'] = '_rudra_keywords';
        $data['page_header'] = ' Keywords';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'filter', 'title', 'status', );
        ...
        $filter = (isset($input['filter']) && $input['filter'] != "") ? $input['filter'] : "";
        if ($filter != "") {
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'keywords/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'keywords/post_actions';
				$action_url = 'keywords/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_keywords_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            ...
            if(isset($_FILES['image']) && $_FILES['image']['name'] != '') 
            {
                ...
                else
                {
					...
					$up_array = array( 'image' => base_url().$bannerpath . '/' . $uploadedImage['file_name']);
					$this->db->where('id', $id);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_keywords').'_'.date('d-m-Y').".csv";
