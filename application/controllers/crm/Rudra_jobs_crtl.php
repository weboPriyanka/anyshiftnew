...
{    public  $default_data = array();   
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'fc_name','nc_name','fo_name','sm_name', 'job_title', 'shift_type', 'job_hours', 'is_premium', 'job_rate', 'job_prem_rate', 'status', );
		...
		$filter_data['searchByFacility'] = html_escape($this->input->post('searchByFacility',true));
		...
		$totalData = $this->rudram->count_table_data($filter_data,$this->full_table,$leftJoin1,$leftJoin2,$leftJoin3,'',$leftJoin4);
		$totalFiltered = $totalData; //Initailly Total and Filtered count will be same
		$rescheck = '';
		$rows = $this->rudram->admin_get_table_data($limit, $start, $orderColumn, $dir, $filter_data,$rescheck,$this->full_table,$leftJoin1,$leftJoin2,$leftJoin3,'',$leftJoin4);
		$rows_count = $this->rudram->count_table_data($filter_data,$this->full_table,$leftJoin1,$leftJoin2,$leftJoin3,'',$leftJoin4);
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				...
				$row->job_prem_rate = "$".$row->job_prem_rate;
				$actions_base_url = 'jobs/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'jobs/post_actions';
				$action_url = 'jobs/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
			...
			$data['facility_categories'] = $fc_category;
			$nurse_categories = $this->db->get_where('rudra_nurse_category',array('status'=>'Active'))->result(); 
			$data['nurse_categories'] = $nurse_categories;
			$shift_managers = $this->db->get_where('rudra_shift_manager',array('fo_id'=>@$data['form_data']->fo_id))->result(); 
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_jobs_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {   $validations = array( 
							array('field' => 'sm_id', 'label' => 'Shift Manager', 'rules' => 'trim|required'), 
							array('field' => 'fc_cat_id', 'label' => 'Facility Category', 'rules' => 'trim|required'), 
							array('field' => 'cg_cat_id', 'label' => 'Care Giver Category', 'rules' => 'trim|required'), 
							array('field' => 'job_title', 'label' => 'Job Title', 'rules' => 'trim|required'), 
							array('field' => 'job_description', 'label' => 'Job Description', 'rules' => 'trim|required'),
						 );
        ...
        {
            $header = array('Id','Category','Nurse Category','Facility Name','Job Title','Rate','Premium Rate','Status');
            $filename = strtolower('rudra_jobs').'_'.date('d-m-Y').".csv";
            $fp = fopen('php://output', 'w');                         
            header('Content-type: application/csv');
            header('Content-Disposition: attachment; filename='.$filename);
            fputcsv($fp, $header);
			$this->db->select('rudra_jobs.id,rudra_facility_category.fc_name,rudra_nurse_category.nc_name,concat(rudra_facility_owner.fo_fname," ",rudra_facility_owner.fo_lname) as fo_name,rudra_jobs.job_title,rudra_jobs.job_rate,rudra_jobs.job_prem_rate,rudra_jobs.status');
			...
			$this->db->join('rudra_facility_owner', 'rudra_facility_owner.id = rudra_jobs.fo_id');
