...
class Rudra_nurse_category_crtl extends CI_Controller
{    public  $default_data = array();
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_nurse_category';
        ...
        $this->load->model('rudra_nurse_category_rudra_model','rudram');
	...
	//Rudra_nurse_category_crtl ROUTES
        ...
        $route['rudra_nurse_category'] = $crud_master . 'index';
        $route['rudra_nurse_category/index'] = $crud_master . 'index';
        $route['rudra_nurse_category/list'] = $crud_master . 'list';
        $route['rudra_nurse_category/post_actions/(:any)'] = $crud_master.'post_actions/$1';
	**************/
	...
	{   
		...
		if(!empty($ajax_data)){
			...
			$message = $this->load->view('crm/ajax/NCatName',$this->data,TRUE);
			$response = array(
                'status' => 'success',
                'message' => $message
            );
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' Nurse Category';                        
        $data['page_template'] = '_rudra_nurse_category';
        $data['page_header'] = ' Nurse Category';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'nc_name', 'status', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'nurse-category/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'nurse-category/post_actions';
				$action_url = 'nurse-category/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_nurse_category_form", $data, TRUE);
            $this->return_data['data']['form_data'] = $html;
        }
        // Post Methods
        //Update Codes
        if($action == "update_data")
        {   if($this->input->post('nc_name') != $this->input->post('original_name')) {
			   $is_unique =  '|is_unique[rudra_nurse_category.nc_name]';
			} else {
			   $is_unique =  '';
			}
			$validations = array(
							array('field' => 'nc_name', 'label' => 'Nurse Category Name', 'rules' => 'trim|required'.$is_unique), 
						 );
        ...
        if($action == "insert_data")
        {
            $id = 0;
			$validations = array(
							array('field' => 'nc_name', 'label' => 'Nurse Category Name', 'rules' => 'trim|required|is_unique[rudra_nurse_category.nc_name]'), 
						 );
        $this->form_validation->set_rules($validations);	
        if ($this->form_validation->run() == false) {
            $this->return_data = array(
        ...
        if($action == "export_data")
        {
            ...
            foreach($json_cols as $ck => $cv)
            {
				if($cv->col_name=='nc_name'){
						$header[] = 'Nurse Category';
					}else{
						$header[] = $cv->list_caption;
					}
                
            }
            $filename = strtolower('rudra_nurse_category').'_'.date('d-m-Y').".csv";
