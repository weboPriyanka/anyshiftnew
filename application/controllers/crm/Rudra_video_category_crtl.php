...
class Rudra_video_category_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_video_category';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_video_category_rudra_model','rudram');
    ...
    /***********
	//Rudra_video_category_crtl ROUTES
        $crud_master = $crm . "Rudra_video_category_crtl/";
        $route['rudra_video_category'] = $crud_master . 'index';
        $route['rudra_video_category/index'] = $crud_master . 'index';
        $route['rudra_video_category/list'] = $crud_master . 'list';
        $route['rudra_video_category/post_actions/(:any)'] = $crud_master.'post_actions/$1';
	**************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Video Category';                        
        $data['page_template'] = '_rudra_video_category';
        $data['page_header'] = ' Video Category';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_video_category/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_video_category/post_actions';
				$action_url = 'rudra_video_category/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_video_category_form", $data, TRUE);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_video_category').'_'.date('d-m-Y').".csv";
