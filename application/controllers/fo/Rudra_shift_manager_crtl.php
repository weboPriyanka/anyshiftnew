...
class Rudra_shift_manager_crtl extends CI_Controller
{   public  $default_data = array();     
    public function __construct()
    {
	   ...
	   $this->default_data = array('data'=>$data);
        ...
        $this->base_id = 0;
        $this->load->model('rudra_shift_manager_rudra_model','rudram');
		$this->load->helper('User_helper'); 
		if(empty($this->session->userdata('rudra_fo_id'))){
			redirect('fo-login');
		}
    }
	...
	{   
        ...
        $ajax_data = $this->rudram->search($find,$whereSearch);
		if(!empty($ajax_data)){
			$this->data =array('ajax_data'=> $ajax_data);
			$message = $this->load->view('facility/ajax/ShiftManagerName',$this->data,TRUE);
			$response = array(
                'status' => 'success',
                'message' => $message);
		}else{
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' Shift Manager';                        
        $data['page_template'] = 'rudra_shift_manager';
        $data['page_header'] = 'Shift Manager';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'sm_name', 'sm_mobile', 'sm_email', 'sm_password','status',);
		...
		$leftJoin1 = "fo_id";
		$whereSearch->Value = $this->session->userdata('rudra_fo_id');
		$filter_data['id'] = html_escape($this->input->post('sm_id',true));
		...
		$totalData = $this->rudram->count_table_data($filter_data,$this->full_table,$leftJoin1,$whereSearch);
		$totalFiltered = $totalData; //Initailly Total and Filtered count will be same
		$rescheck = '';
		$rows = $this->rudram->get_table_data($limit, $start, $orderColumn, $dir, $filter_data,$rescheck,$this->full_table,$leftJoin1,$whereSearch);
		$rows_count = $this->rudram->count_table_data($filter_data,$this->full_table,$leftJoin1,$whereSearch);
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'facility-shift-manager/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'facility-shift-manager/post_actions';
				$action_url = 'facility-shift-manager/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("facility/forms/_ajax_rudra_shift_manager_form", $data, TRUE);
        ...
        {
			if($this->input->post('sm_email') != $this->input->post('original_email')) {
			   $is_unique =  '|is_unique[rudra_shift_manager.sm_email]';
			} else {
			   $is_unique =  '';
			}
			$validations = array(array('field' => 'sm_fname', 'label' => 'Shift Manager First Name', 'rules' => 'trim|required'),
							array('field' => 'sm_lname', 'label' => 'Shift Manager Last Name', 'rules' => 'trim|required'), 
							array('field' => 'sm_mobile', 'label' => 'Mobile Number', 'rules' => 'trim|required|numeric|min_length[9]|max_length[11]'), 
							array('field' => 'sm_email', 'label' => 'Shift Manager Email', 'rules' => 'trim|required'.$is_unique));
        ...
        if ($this->form_validation->run() == false) {
			...
			'status' => 'error',
				...
				{
					$id = $_POST['id'];
					$updateArray = array( 'id' => $this->input->post('id',true),
					 'fo_id' => $fo_id,
        ...
        if($action == "insert_data")
        {
			...
			$validations = array(array('field' => 'sm_fname', 'label' => 'Shift Manager First Name', 'rules' => 'trim|required'),
							array('field' => 'sm_lname', 'label' => 'Shift Manager Last Name', 'rules' => 'trim|required'), 
							array('field' => 'sm_mobile', 'label' => 'Mobile Number', 'rules' => 'trim|required|numeric|min_length[9]|max_length[11]'), 
							array('field' => 'sm_email', 'label' => 'Shift Manager Email', 'rules' => 'trim|required|is_unique[rudra_shift_manager.sm_email]'));
        $this->form_validation->set_rules($validations);	
        if ($this->form_validation->run() == false) {
            $this->return_data = array(
			'status' => 'error',
				...
				{ 
					...
					$insertArray = array( 'id' => $this->input->post('id',true),
					 'fo_id' => $fo_id,
					 'sm_fname' => $this->input->post('sm_fname',true),
					 'sm_lname' => $this->input->post('sm_lname',true),
					 'sm_mobile' => $this->input->post('sm_mobile',true),
					 ...
					 'sm_password' => $this->input->post('sm_password',true),
					 'status' => $this->input->post('status',true));
					$this->db->insert($this->full_table,$insertArray);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_shift_manager').'_'.date('d-m-Y').".csv";
