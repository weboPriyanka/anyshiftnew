...
class Rudra_videos_apis extends CI_Controller
{                   
    ...
    **********************************/
    ...
    public function rudra_rudra_videos($param1)
    {
        $call_type = $param1;
        $res = array();
        if($call_type == 'put')
    ...
    {     
        if(!empty($_POST))
        {
            ...
            { 
                ...
                $query = "SELECT rudra_videos.id,rudra_videos.video_url,rudra_videos.status,rudra_videos.added_on,rudra_videos.updated_on,rudra_video_category.cat_name,rudra_video_subcategory.subcat_name FROM $this->table LEFT JOIN rudra_video_category ON $this->table.cat_id = rudra_video_category.id LEFT JOIN rudra_video_subcategory ON $this->table.subcat_id = rudra_video_subcategory.id ORDER BY id DESC LIMIT $start_index , $per_page";
                ...
                if(!empty($result))
                {
                    ...
                    foreach($result as $res)
                    {
                        ...
                        $res->updated_on_custom_date = date('d-M-Y',strtotime($res->updated_at));
                        $res->updated_on_custom_time = date('H:i:s A',strtotime($res->updated_at));*/
						$res->video_url = base_url('app_assets/uploads/videos/').$res->video_url;
                        $list[] = $res;
                    }
                    $this->chk = 1;
                    $this->msg = 'Paged Data';
