					   ...
					   <div class="row">
					    <div class="form-group col-sm-3 ">
                            ...
                            <div class="suggetions_listings" style="display:none;">
                               <ul class="list-group">
										<li class="list-group-item">No Care Giver Found</li>
								</ul>
                        </div>
						</div>
                        <div class="form-group col-sm-3 ">
                    ...
                    <div class="row">
                        ...
                        <h5><?= $page_header ?></h5>
                            ...
                            <div class="col-sm-7 text-right">
                                <!--<button onclick="static_form_modal('rudra_care_giver/post_actions/get_data?id=0','rudra_care_giver/post_actions/insert_data','md','Update Details')" type="button" class="btn btn-glow-success btn-info" title="" data-toggle="tooltip" data-original-title="Add New Item" aria-describedby="tooltip651610">New</button>-->&nbsp;
                                <a href="<?php echo site_url('care-giver/post_actions/export_data');?>" type="button" class="btn btn-glow-danger btn-warning" title="" data-toggle="tooltip" data-original-title="Export CSV" aria-describedby="tooltip651610">CSV</a>
...
<!--- FORM Modal Ends ---->
  <div class="md-modal md-effect-11 dynamic-modal" id="form_modal_rudra_care_giver">
       <div class="modal-content">
           <div class="modal-header theme-bg2 ">
               <h5 class="modal-title text-white" id="heading_rudra_care_giver">Loading...</h5>
               <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
           </div>
           <form id="frm_rudra_care_giver" method="post" enctype="multipart/form-data">
               <div class="modal-body" id="modal_form_data_rudra_care_giver"> </div>
...
$("#filter").submit(function(e) {
   ...
   fill_server_data_table(search_json,cg_id, searchByCare,start_date,end_date);
});
$.fn.dataTable.ext.errMode = 'throw';
var usrTable;
fill_server_data_table(search_json);
function fill_server_data_table(search_json,cg_id = "", searchByCare = "",start_date ="",end_date =""){
...
"serverSide": true,
 ...
 responsive: true,
"ajax": {
	"url": "<?php echo base_url('care-giver/list') ?>",
	"dataType": "json",
	"type": "POST",
	"data": {
	'searchBy': searchByCare,
	'id': cg_id,
...
function static_form_modal(data_url, action_url, mtype, heading) {
            $("#form_modal_rudra_care_giver").modal();
            $("#form_modal_rudra_care_giver").addClass('md-show');
            $("#heading_rudra_care_giver").text(heading);
            $("#frm_rudra_care_giver").attr("action", '<?= base_url() ?>' + action_url);
            $.ajax({
                type: 'POST',
                ...
                success: function(data) {
                    ...
                    if (response.status) {
                        ...
                        $("#modal_form_data_rudra_care_giver").html(jsonData.form_data);
...
$("#frm_rudra_care_giver").submit(function(e) {
            e.preventDefault();
            //$(".btn-modal-form").prop('disabled', true);
            //$(".btn-modal-form").html('Wait...');
            var formData = new FormData($("#frm_rudra_care_giver")[0]);
            var action_url = $("#frm_rudra_care_giver").attr("action");
            $.ajax({
                type: 'POST',
                url: action_url,
                data: formData,
