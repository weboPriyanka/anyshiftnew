...
class Rudra_nurse_category_rudra_model extends CI_Model               
{                  
	...
	public function search($find){
		$count = 0;
		$this->db->group_start();
		$this->db->like('nc_name',$find);
		$this->db->group_end();
		$res = $this->db->get('rudra_nurse_category');
		//echo $this->db->last_query();die;
		$count = $this->db->count_all_results('rudra_nurse_category');	
		if ($count == 1) {
	...
	$query = " SELECT TBL.id, TBL.nc_name, TBL.status, TBL.added_on" ;
	$query .= "  FROM $table   "; 
	$where = " WHERE 1 = 1 ";
	...
	{
		$where .=  " AND (  TBL.nc_name LIKE '%".$filter_data['searchBy']."%')";
	}
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.nc_name LIKE '%".$filter_data['general_search']."%'";
